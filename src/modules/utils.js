...
export const getAddressesByBusinessLocations = ({
  businessLocations = [], address, city, state
}) => {
  const uniqueLocations = businessLocations
    .filter((location, i) => (
      businessLocations.findIndex(item => item.city === location.city) === i
    ));

  // return all cities and states if there's more than one location
  if (businessLocations && businessLocations.length > 1) return uniqueLocations.map(location => `${location.city}, ${location.state}`).join('\n');

  // return the unique location if there's one location
  if (uniqueLocations[0] && !(address && state && city)) return `${uniqueLocations[0].address}, ${uniqueLocations[0].city}, ${uniqueLocations[0].state}`;
